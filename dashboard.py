...
if actions_updated: save_json(actions, actions_path)
...
<div style='padding:1rem; margin-bottom:1rem; border-radius:8px; background:#f9f9f9;'>
  ...
  <b>Actual:</b> {'$'+format(actual,',.2f') if actual else '_awaiting_'}<br>
  <b>Accuracy:</b> {icon}
</div>
""", unsafe_allow_html=True)
    if not shown:
        st.info("No ML predictions yet. Run analyze.py to generate them.")
else:
